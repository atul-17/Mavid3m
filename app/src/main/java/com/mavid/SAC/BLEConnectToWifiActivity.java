...
import static com.mavid.Constants.Constants.CONFIGURED_DEVICE_FOUND;

public class BLEConnectToWifiActivity extends BaseActivity {
    ...
    private Dialog alertConnectingtoNetwork;
    ...
    private void initViews() {
        ...
        homeIcon = (ImageButton) findViewById(R.id.homebutton);
        homeIcon.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                startActivity(new Intent(BLEConnectToWifiActivity.this, MavidHomeTabsActivity.class));
                finish();
                //   luciControl.SendCommand(MIDCONST.MID_REMOTE_UI, GET_HOME, LSSDPCONST.LUCI_SET);
//                if(MavidApplication.isSacConfiguredSuccessfully){
//                    if(MavidNodes.getInstance().getDeviceCount()==0){
//                        startActivity(new Intent(WifiYesAlexaLoginNoHomeScreen.this, NoDeviceConfigureScreen.class));
//                        finish();
//                    }
//                    else{
//                        startActivity(new Intent(WifiYesAlexaLoginNoHomeScreen.this, DeviceListFragment.class));
//                        finish();
//                    }
//                }
//                else{
...
//                }
    ...
    private void handleAlexaRefreshTokenStatus(String current_ipaddress, String refreshToken) {
        if (refreshToken != null && !refreshToken.isEmpty()) {
            /*not logged in*/
            Intent i = new Intent(BLEConnectToWifiActivity.this, AlexaThingsToTryDoneActivity.class).setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
            i.putExtra("speakerIpaddress", current_ipaddress);
            i.putExtra("fromActivity", MavidHomeTabsActivity.class.getSimpleName());
            startActivity(i);
            finish();
        } else {
            Intent newIntent = new Intent(BLEConnectToWifiActivity.this, AlexaSignInActivity.class).setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
    ...
    public boolean isThisDeviceLookingFor(DeviceInfo deviceInfo) {
        if (deviceInfo != null) {
            try {
                ...
                String lookForDevice = MavidApplication.lookFor.substring(MavidApplication.lookFor.length() - 5);

                if (lookForDevice.contains(amr)) {
                    return true;
                }
            } catch (Exception e) {
                e.printStackTrace();
...
//            Bundle bundle = getIntent().getBundleExtra(Constants.INTENTS.SAC_PARAMS);
        ssid = getIntent().getExtras().getString(BundleConstants.SACConstants.SSID);
        password = getIntent().getExtras().getString(BundleConstants.SACConstants.PASSPHRASE);
        int sec = getIntent().getExtras().getInt(BundleConstants.SACConstants.NETWORK_TYPE);
        if (sec == 0) {
            security = OPEN;
        } else {
            security = PSK;
        }
        lookFor = getIntent().getExtras().getString(Constants.INTENTS.MACADDRESS);
    ...
    Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == Constants.HTTP_POST_SOMETHINGWRONG) {
                ...
                if (getconnectedSSIDname(BLEConnectToWifiActivity.this).equalsIgnoreCase(ssid)) {
...
//                Toast.makeText(BLEConnectToWifiActivity.this, getString(R.string.httpPostFailed), Toast.LENGTH_LONG).show();
                ...
                if (mWifi.isConnected()) {
                    if (getconnectedSSIDname(BLEConnectToWifiActivity.this).contains(ssid)) {
                        mHandler.sendEmptyMessage(Constants.CONNECTED_TO_MAIN_SSID_SUCCESS);
                        LibreLogger.d(this, "suma in connectTowifi wifisucess");


                    } else if (!getconnectedSSIDname(BLEConnectToWifiActivity.this).contains(Constants.WAC_SSID) || !getconnectedSSIDname(BLEConnectToWifiActivity.this).contains(Constants.WAC_SSID2)
                            && !getconnectedSSIDname(BLEConnectToWifiActivity.this).contains("<unknown ssid>")) {
    ...
    public void buildSnackBar(String message) {
        CafeBar.Builder builder = CafeBar.builder(BLEConnectToWifiActivity.this);
    ...
    public void alertBoxForNetworkOff() {
       /* if (!BLEConnectToWifiActivity.this.isFinishing()) {
            AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(
                    BLEConnectToWifiActivity.this);
    ...
    private void showDialogifDeviceNotFound(final String Message) {
        ...
        if (!BLEConnectToWifiActivity.this.isFinishing()) {
            ...
            if (alert == null) {
                ...
                alert = new Dialog(BLEConnectToWifiActivity.this);
            ...
            btn_ok.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    if (Message.contains(getString(R.string.noDeviceFound))) {
                        Intent ssid = new Intent(BLEConnectToWifiActivity.this,
...
//            AlertDialog.Builder builder = new AlertDialog.Builder(BLEConnectToWifiActivity.this);
//
...
//                            alert = null;
//                            if (Message.contains(getString(R.string.noDeviceFound))) {
////                                Intent n1 = new Intent(getApplicationContext(),NoDeviceConfigureScreen.class);
////                                startActivity(n1);
////                                finish();
//                                Intent ssid = new Intent(BLEConnectToWifiActivity.this,
//                                        MavidHomeTabsActivity.class).setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
...
//            }
    ...
    private void showAlertDialogForConnectingToWifi() {
        if (!BLEConnectToWifiActivity.this.isFinishing()) {
            ...
            alertWrongNetwork = new Dialog(BLEConnectToWifiActivity.this);
    ...
    private void showAlertDialogForConnectedToWrongNetwork() {
        if (!BLEConnectToWifiActivity.this.isFinishing()) {
            ...
            String Message = String.format(getString(R.string.newrestartApp),
                    getconnectedSSIDname(BLEConnectToWifiActivity.this), ssid);

            tv_alert_message.setText(Message);

            btn_ok.setText(R.string.gotoSettings);
...
//
    ...
    private void showAlertDialogForClickingWrongNetwork() {
        if (!BLEConnectToWifiActivity.this.isFinishing()) {
            ...
            alertConnectingtoNetwork = new Dialog(BLEConnectToWifiActivity.this);

            alertConnectingtoNetwork.requestWindowFeature(Window.FEATURE_NO_TITLE);

            alertConnectingtoNetwork.setContentView(R.layout.custom_single_button_layout);

            alertConnectingtoNetwork.setCancelable(false);

            tv_alert_title = alertConnectingtoNetwork.findViewById(R.id.tv_alert_title);

            tv_alert_message = alertConnectingtoNetwork.findViewById(R.id.tv_alert_message);

            btn_ok = alertConnectingtoNetwork.findViewById(R.id.btn_ok);
